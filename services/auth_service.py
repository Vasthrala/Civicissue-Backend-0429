...
def signup(db: Session, data: SignupRequest) -> dict:
    ...
    try:
        ...
        db.commit()
        db.refresh(user)
        logger.info(f"OTP {otp_code} created and stored for user {user.email}")
    except Exception as e:
        db.rollback()
        logger.error(f"Error storing OTP for user {data.email}: {str(e)}")
        raise ValueError(f"Failed to generate verification code: {str(e)}")
...
def resend_otp(db: Session, email: str) -> dict:
    ...
    try:
        ...
        db.add(otp_record)
        db.commit()
        logger.info(f"OTP {otp_code} resent and stored for user {user.email}")
    except Exception as e:
        db.rollback()
        logger.error(f"Error resending OTP for user {email}: {str(e)}")
        raise ValueError(f"Failed to resend verification code: {str(e)}")
...
def forgot_password(db: Session, data: ForgotPasswordRequest) -> dict:
    ...
    try:
        ...
        db.add(otp_record)
        db.commit()
        logger.info(f"Password reset OTP {otp_code} stored for user {data.email}")
    except Exception as e:
        db.rollback()
        logger.error(f"Error storing password reset OTP for user {data.email}: {str(e)}")
        raise ValueError(f"Failed to generate reset code: {str(e)}")
