...
def get_stats(db: Session, user: User = None) -> dict:
    """Return aggregate complaint statistics. Filtered by user if not admin."""
    # Base query
    ...
    if user:
        if user.role == "citizen":
            base_query = base_query.filter(Complaint.citizen_id == user.id)
        elif user.role == "officer":
            base_query = base_query.filter(Complaint.assigned_officer_id == user.id)
    
    total = base_query.count()

    # Counts by status
    def _count_status(s: str) -> int:
        return base_query.filter(Complaint.status == s).count()
    ...
    # By category
    cat_rows = (
        base_query.with_entities(Complaint.category, func.count(Complaint.id))
    ...
    # By severity
    sev_rows = (
        base_query.with_entities(Complaint.severity_level, func.count(Complaint.id))
    ...
    recent_7_days = (
        base_query.filter(Complaint.created_at >= seven_days_ago)
        .count()
    )
